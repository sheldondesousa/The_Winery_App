...
- Clear and Clear All sit on the same line as the most recent date heading (for example, TODAY), each with a black-at-10%-opacity background for emphasis. Clear reveals an empty checkbox to the left of every History entry for manual selection; Clear All reveals the checkboxes with every entry selected. Whichever action is tapped changes its label to Cancel and disables the other action. Tapping Cancel exits selection and enables both actions again. A floating trash-can icon with a `Delete` label appears above the bottom navigation during selection, uses black when active, remains muted and disabled without a selection, and removes the selected local records when tapped.
