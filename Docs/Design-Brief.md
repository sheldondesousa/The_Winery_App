...
- Clear and Clear All sit on the same line as the most recent date heading (for example, TODAY), separated by a short vertical hairline. Clear reveals an empty checkbox to the left of every History entry for manual selection and changes to Cancel while selection mode is open. Clear All reveals the checkboxes with every entry selected. A black floating Delete button appears above the bottom navigation during selection, remains muted and disabled without a selection, and removes the selected local records when tapped.
