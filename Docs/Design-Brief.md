...
- The shared header places the History icon and 24sp title on the left and the right-aligned Uncork/AI SOMMELIER brand lockup on the same line. Entries use the wine name as the primary serif line, followed by region, `Request:` plus locally extracted user-request keywords (for example country, region, variety, color, body, acidity, tannin, and flavor), and a trailing chevron. AI conversation text is not shown in History, and suggestion attributes not present in the user's request are not added as request keywords.
- The empty History content area is intentionally blank for now.
- A top action row provides Select and Delete. Select reveals an empty checkbox on every History entry and permits one or multiple selections; Delete remains disabled until at least one entry is selected, then permanently removes the selected local records.
