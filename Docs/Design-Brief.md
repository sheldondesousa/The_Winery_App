...
- Wine Profile shows `Suggested pairing` as an upfront detail field. A circular, center-aligned control sits at the bottom of the page. Its off state is burgundy with the label `Save`; tapping it saves the wine locally to Favorites and changes it to a lighter muted treatment labeled `Saved`. Tapping again removes the wine and restores `Save`. Favorites list tiles do not show heart icons. Personal rating is read-only on Wine Profile; without a prior rating, the page says `You have not tried this wine`.
