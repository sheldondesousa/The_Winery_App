...
- Wine Profile shows `Suggested pairing` as an upfront detail field. Its former pairing and favorites action buttons are removed; a right-aligned `Add to Favorite` tag beside AI Confidence toggles on and off. On saves the wine locally to Favorites and off removes it. Personal rating is read-only on Wine Profile; without a prior rating, the page says `You have not tried this wine`.
