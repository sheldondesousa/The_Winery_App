...
**Later the same day, the dedicated second-stage "complete the profile" Gemma call that could have backfilled this field on revisit (`loadProfile()`, `profile_system_instruction.txt`) was found to be unreachable dead code — every live card-creation path (Chat search, Guided Selection) already marks `profileComplete = true` the moment a card is published, so the reload never actually ran — and was deleted outright rather than left in place. This removes "accept the added latency of a dedicated summary call" as a live option for the open decision above; reviving it would mean rebuilding the mechanism, not just re-enabling it. See Bug-Log.md #6 and Trade-offs.md for detail.
...
- **GS-AC7:** Gemma names one real, known wine per recommendation, consistent with all selected criteria — as of 25 September 2026 the prompt (`guided_instruction.txt`) requires a genuine `name` "from knowledge," matching Chat's card-search prompt, rather than describing a plausible category with no named bottle. `GuidedCriteria`'s fields were migrated from `Set<String>` to plain `String` the same day (see Trade-offs.md, item 8, superseded): the type itself now enforces single-select instead of relying on UI convention over a structurally multi-value type, and the now-unreachable `toggled()` multi-select helper was deleted. It may choose an appropriate variety and fill unselected attributes with plausible model-generated values or Unknown. Validate every selected field, including Type, before displaying the recommendation. Reject contradictory/malformed output with a retryable error — including the model echoing the JSON schema's own field name back as a value, or hallucinating a markdown/URL citation in place of a name (see Bug-Log.md #7) — a valid empty recommendation array is an empty result. Do not invent a winery, critic score, or review.
...
Status checked against the working code on 25 September 2026, including the Chat recommendation-flow rework and per-source master-card UI redesign described in Sections 4 and 6 (PR #17 and PR #18), the progressive card-streaming/timing-diagnostics performance rework, and the card-consistency/Gemma-hardening/`GuidedCriteria` cleanup described below (PR #19, all three merged to `Master`). A checked item is implemented and has passed the computer-only build or automated checks. Partially complete items have working foundations but still require the work stated beside them.
...
- [ ] **Product decision needed:** the Profile Page's `Summary` section is now unconditionally `Unknown` for every Chat-originated Gemma card, since `summary` was deliberately dropped from Chat's card-search prompt to cut latency (see Section 4, Section 6 AC4a, and the "Dropping `summary`…" trade-off). Find's Gemma recommendation is unaffected — it still requests and displays `summary` normally. The dedicated second-stage call that could have backfilled this field on revisit has since been deleted as dead code (see Section 4 and Bug-Log.md #6), so "accept the added latency of a dedicated summary call" is no longer a live option without rebuilding that mechanism from scratch. Needs a decision: leave the section empty/relabeled for Chat cards, or rebuild a backfill path.
